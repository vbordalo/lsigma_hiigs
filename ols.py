...
def leastsq(X, y, method=5):
    ...
    covb12 = np.sum(xdif*ydif*(ydif-beta2*xdif)*(ydif-beta1*xdif)
                    )/(beta1*sxx**2)  # Note Table 1
    ...
    if method == 1:
        ...
        bvar = np.sqrt(bvar1)
        avar = np.sqrt(
            np.sum((ydif-beta*xdif-n*xbar*xdif*(ydif-beta*xdif)/sxx)**2)/n**2)  # Eq. 9,11
    if method == 2:
        """
        OLS(X|Y)
        """
        ...
        bvar = np.sqrt(bvar2)
        avar = np.sqrt(
            np.sum((ydif-beta*xdif-n*xbar*ydif*(ydif-beta*xdif)/sxy)**2)/n**2)  # Eq. 9,15
    if method == 3:
        """
        OLS bisector
        """
    ...
    # The intercept, rms, rho
    alpha = ybar - beta*xbar
    yhat = alpha + beta*x
    rms = np.sqrt(sum((y-yhat)**2)/(n-k))  # in units of Y
    rho, _ = st.pearsonr(y, yhat)

    return (alpha, avar, beta, bvar, rms, rho)
